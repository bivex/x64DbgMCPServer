...
internal sealed class PluginSessionProxy : IPluginSession {
    ...
    public PluginSessionProxy ( WaitCallback implChangedCallback )
    {
        ...
        _session = ( PluginSession ) _appDomain.CreateInstanceAndUnwrap ( typeof ( PluginSession ).Assembly.GetName().Name,
                   typeof ( PluginSession ).FullName );
    ...
    private async Task WatchForImplChangeAsync()
    {
        ...
        try
        {
            fsw = new FileSystemWatcher ( Path.GetDirectoryName ( PluginMain.ImplAssemblyLocation ),
                                          Path.GetFileName ( PluginMain.ImplAssemblyLocation ) );
        }
        catch
        {
        ...
        using ( fsw )
        {
            ...
            try
            {
                await changedTcs.Task.ConfigureAwait ( false );
            }
            catch ( Exception ex )
    ...
    private bool StopCore ( PluginSession session )
    {
        try
        {
            ...
            if ( Task.WhenAny ( pendingTasks, Task.Delay ( 5000 ) ).ConfigureAwait ( false ).GetAwaiter().GetResult() ==
                    pendingTasks )
            {
