...
public static class Pattern {
    ...
    private static extern nuint Script_Pattern_FindMem ( nuint start,
            nuint size, [MarshalAs ( UnmanagedType.LPUTF8Str )] string pattern );

    public static nuint FindMem ( nuint start, nuint size, string pattern ) => Script_Pattern_FindMem ( start, size,
            pattern );
}
}
